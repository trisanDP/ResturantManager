...
namespace UnityAsyncAwaitUtil
{
    public class AsyncUtilTests : MonoBehaviour
    {
        ...
        async Task RunWwwAsync() {
            Debug.Log("Downloading asset bundle using UnityWebRequest");

            using(var request = UnityWebRequest.Get(AssetBundleSampleUrl)) {
        ...
        async Task<byte[]> DownloadRawDataAsync(string url) {
            using(var request = UnityWebRequest.Get(url)) {
                ...
                if(request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError) {
                    Debug.LogError($"Error downloading data from {url}: {request.error}");
                    return null;
                }
